 ...
 * 
 */
public class FlatParameterHandler extends InterfaceParametersHandler
{
    private static Logger            logger        = org.slf4j.LoggerFactory.getLogger(FlatParameterHandler.class);

    private Map<String, MappingData> paramMappings = null;

    /**
     * @see it.greenvulcano.gvesb.adapter.http.formatters.handlers.InterfaceParametersHandler#initSpecific(org.w3c.dom.Node)
     */
    @Override
    protected void initSpecific(Node specificParamHandlerSection) throws AdapterHttpConfigurationException
    {
        try {
            ...
            for (int i = 0; i < mappingNodes.getLength(); i++) {
                ...
                mappingData.offset = XMLConfig.getInteger(currMapping, "@Offset", 0);
                mappingData.length = XMLConfig.getInteger(currMapping, "@Length", Integer.MAX_VALUE);
                ...
                logger.debug("initSpecific - mappingVariable: " + mappingVariable + " - MappingData: " + mappingData);
                paramMappings.put(mappingVariable, mappingData);
            }
        }
        catch (XMLConfigException ex) {
            logger.error("initSpecific - Error while using XMLConfig: " + ex);
            throw new AdapterHttpConfigurationException("GVHTTP_ADAPTER_XML_CONFIG_ERROR", ex);
        }
        catch (Throwable ex) {
            logger.error("initSpecific - Generic error during FlatParameterHandler initialization: " + ex);
            throw new AdapterHttpConfigurationException("GVHTTP_ADAPTER_RUNTIME_ERROR", new String[][]{
                    {"phase", "FlatParameterHandler object initialization"}, {"errorName", "" + ex}}, ex);
        }
    }
    ...
    protected GVBuffer buildGVBuffer(Object input, GVBuffer previous) throws DataHandlerException
    {
        ...
        Document doc = null;
        try {
            Iterator<String> iter = paramMappings.keySet().iterator();
            while (iter.hasNext()) {
    ...
    protected String buildHttpParam(GVBuffer input, String previous) throws HttpParamHandlerException
    {
        ...
        try {
            Iterator<String> iter = paramMappings.keySet().iterator();
            while (iter.hasNext()) {
                String currSourceVar = iter.next();
                ...
                if (currSourceVar != null) {
                    if (currSourceVar.equals(AdapterHttpConfig.MAPPING_VARIABLE_VALUE_GVBUFFER)) {
                        ...
                        outputHttpParam = getStringValue(gvBufferAccess.getFieldAsString(currSourceVar), mappingData);
                    }
                    else {
            ...
            throw new HttpParamHandlerException("GVHTTP_RUNTIME_ERROR", new String[][]{
                    {"phase", "HTTP parameter value setting"}, {"errorName", "" + exc}}, exc);
        }
    }
