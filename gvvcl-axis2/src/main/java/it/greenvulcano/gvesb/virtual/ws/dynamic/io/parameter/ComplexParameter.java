 ...
 *
 */
public class ComplexParameter implements IParameter, IParameterHolder
{
    private ParamDescription desc;
    private List<IParameter> list = new ArrayList<IParameter>();

    /**
     * Creates a new instance of ComplexParameter
     *
     * @param desc
     */
    public ComplexParameter(ParamDescription desc)
    {
        this.desc = desc;
    }
    ...
    {
        writer.writeComplexStart(desc);

        for (IParameter parameter : list) {
            parameter.write(writer);
        }

        writer.writeComplexEnd();
    }
