 ...
 */
public class DateIntervalTriggerBuilder extends BaseTriggerBuilder
{
    ...
    public void initTB(Node node) throws TaskException
    {
        try {
            ...
            repeatInterval = XMLConfig.getInteger(node, "@repeatInterval", 1);
            repeatIntervalUnit = DateIntervalTrigger.IntervalUnit.valueOf(XMLConfig.get(node, "@repeatIntervalUnit"));
            properties.put(TRIGGER_TYPE, "DateIntervalTrigger");
            properties.put(TRIGGER_NAME, name);
        }
        catch (Exception exc) {
            throw new TaskException("Error initializing DateIntervalTriggerBuilder", exc);
        }
    }
